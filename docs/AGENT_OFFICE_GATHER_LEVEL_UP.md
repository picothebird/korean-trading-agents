...
## 0-sexies.26 猷??쇰꺼 / ?쒓컖 援ы쉷 ??

> ?묒꽦: 2026-04-26 (?대쾲 ?몄뀡 ?곗냽). 9媛?梨낆긽??洹몃９?붾맂 3媛?猷?遺꾩꽍???좊줎???섏궗寃곗젙?????쒓컖??寃쎄퀎? ?대쫫 ?쇰꺼 異붽?.

### 寃곗젙: 諛섑닾紐?諛뺤뒪 + 醫뚯긽???쇰꺼

ROOM_ZONES (`RoomLabels.ts`):
| 猷?| 洹몃━??踰붿쐞 | ?됱긽 |
|---|---|---|
| 遺꾩꽍媛??| col 2-12, row 2-10 | ?뚮옉 0x3182f6 |
| ?좊줎??| col 17-27, row 2-7 | 鍮④컯 0xf04452 |
| ?섏궗寃곗젙??| col 17-27, row 9-17 | ?먯＜ 0xa855f7 |

媛?猷?
- ?곸뿭 諛뺤뒪: 諛섑닾紐?fill alpha 0.06 + 1px outline alpha 0.4 (?대떦 猷?而щ윭)
- ?대쫫 ?쇰꺼: 醫뚯긽???덉そ??70횞18px 諛뺤뒪 + ?곗깋 11px bold ?띿뒪??

?됱긽 ?섎?: 媛?猷몄쓽 "二쇰맂 ?뺤꽌"
- 遺꾩꽍??= 李⑤텇??遺꾩꽍 (?뚮옉)
- ?좊줎??= 異⑸룎/?쇱웳 (鍮④컯)
- ?섏궗寃곗젙??= 寃곗젙 (?먯＜, ?섏궗寃곗젙 ?곹깭?됯낵 留ㅼ묶)

### ?뚮뜑 ?쒖꽌 = 源딆씠

`OfficeScene.create()`:
1. `drawDefaultOffice()` ??留덈（ ??쇱쓣 mapLayer (Container) ?덉뿉 add
2. `createRoomLabels(this)` ??猷?諛뺤뒪/?쇰꺼??scene??standalone add
3. `spawnActors()` ??desk props + character

Phaser ?붿뒪?뚮젅??由ъ뒪?몄뿉??媛숈? depth(0)??insertion ?쒖쑝濡?洹몃젮吏? setDepth 紐낆떆 ?놁씠 ?먯뿰?ㅻ읇寃?floor ??room ??desk ??character ?쒖꽌濡??볦엫.

(珥덇린 ?쒕룄?먯꽌 `setDepth(-3)`?쇰줈 留덈（ ?꾨옒濡??몄떆?섎뒗 踰꾧렇 諛쒓껄 ???쒓굅. 媛숈? depth/insertion ???⑦꽩??媛???덉쟾.)

### ?붿옄???섏궗寃곗젙

- **alpha 0.06**: ?됱씠 ?곸뿭???앸퀎???뺣룄濡쒕쭔, 留덈（ ?됱쓣 ?뺣룄?섏? ?딆쓬. ?대뼡 ?뚮쭏?먯꽌??媛?낆꽦 ?좎?.
- **outline alpha 0.4**: ?곸뿭 寃쎄퀎媛 ?댁쭩 ?덉뿉 ?꾩뼱 猷??뺥깭 ?몄떇.
- **?쇰꺼 醫뚯긽??*: 猷??덉そ?쇰줈 ?ㅼ뼱?ㅻ뒗 ?쒖꽑 ?먮쫫. ?ъ슜?먭? 罹먮┃?곕? 蹂대떎媛 ?먯뿰?ㅻ읇寃?猷?而⑦뀓?ㅽ듃 ?몄?.
- **怨좎젙 ROOM_ZONES**: DESK_POSITIONS? 遺꾨━??蹂꾨룄 ?곸닔. ?ν썑 誘몃땲留듭뿉??猷?而щ윭瑜??쒖떆?섎뒗 ???ъ궗??媛??

### 肄붾뱶 蹂寃?

| ?뚯씪 | 蹂寃?|
|---|---|
| `frontend/src/components/game/RoomLabels.ts` (?좉퇋) | `ROOM_ZONES` ?곸닔 + `createRoomLabels(scene) ??RoomLabelsHandle` ?⑺넗由? 猷몃퀎 3媛?GameObject (box/labelBg/label) ?앹꽦. |
| `frontend/src/components/game/OfficeScene.ts` | `roomLabels: RoomLabelsHandle \| null` ?꾨뱶. `create()`?먯꽌 `drawDefaultOffice` 吏곹썑 `createRoomLabels` ?몄텧. bootText 媛깆떊. |
...
誘몃땲留듭뿉 猷??됱쓣 ?낃쾶 ?쒖떆??以??꾩썐 ?쒖젏?먯꽌??猷?援ы쉷 ?몄떇 媛?? ?ㅼ쓬 ?④퀎 ?꾨낫.



---

## 0-sexies.27 HUD ?먯씠?꾪듃 移댁슫????

> ?묒꽦: 2026-04-26 (?대쾲 ?몄뀡 ?곗냽). 罹붾쾭??醫뚯긽?⑥뿉 ?쒖꽦/?꾨즺/?湲??먯씠?꾪듃 ?섎? ?ㅼ떆媛??쒖떆.

### ?숈옉

- thoughts 諛곗뿴??role蹂?留덉?留?status濡??섏썝 ??active/done/idle 遺꾨쪟
- thoughts???깆옣?섏? ?딆? 9 role? idle濡??⑹궛
- active > 0?대㈃ ?ㅻ뜑 ?먯씠 ?뚮옉 + 1.2珥??꾩뒪, ?꾨땲硫??뚯깋
- styling: 罹붾쾭??醫뚯긽??absolute, 11px ?고듃, ??諛곌꼍 + ?낆? 蹂대뜑

### 遺꾨쪟 湲곗?

| 遺꾨쪟 | status 吏묓빀 |
|---|---|
| ?쒖꽦 | thinking / analyzing / debating / deciding |
| ?꾨즺 | done |
| ?湲?| idle + 誘몃벑??role |
...
| ?뚯씪 | 蹂寃?|
|---|---|
| `frontend/src/components/game/AgentCounter.tsx` (?좉퇋) | useMemo濡?移댁슫??吏묎퀎, ?꾩뒪 ?좊땲硫붿씠???몃씪??`<style>`. |
| `frontend/src/components/game/PhaserCanvas.tsx` | `showHud=true`????AgentCounter 異붽? ?뚮뜑 |

### ?붿옄???섏궗寃곗젙

- **湲濡쒕쾶 CSS ?ㅼ뿼 諛⑹?**: keyframes瑜?而댄룷?뚰듃 ?대? `<style>`濡??뺤쓽. ??踰덈쭔 留덉슫?몃릺??HUD???덉쟾.
- **Counter??罹붾쾭???대? absolute**: ?곷떒 ?섏씠吏 ?ㅻ뜑??異붽??섏? ?딄퀬 PhaserCanvas ?먯껜??遺李⑺빐 而댄룷?뚰듃 ?⑥쐞 ?묒쭛???좎?.
- **active ?꾩뒪留??좊땲硫붿씠??*: ?꾩껜 ?됱씠 源쒕묀?대㈃ 媛?낆꽦 ?⑥뼱吏????먮쭔 媛뺤“.
...
罹붾쾭??4紐⑥꽌由?紐⑤몢 ?뺣낫濡?梨꾩썙吏?
- ??醫뚯긽?? AgentCounter (?쒖꽦/?꾨즺/?湲?
- ???곗긽?? HudControls (+/????
- ??醫뚰븯?? bootText (Phaser ?대? ???④퀎 ?쇰꺼)
- ???고븯?? Minimap (猷????ㅻ쾭?덉씠 ?ы븿)

### ?ㅼ쓬

- **AgentInspector ?⑤꼸 媛쒖꽑**: ?≫꽣 ?대┃ ???곸꽭 ?⑤꼸 (page.tsx ?덈꺼?먯꽌 onAgentClick 泥섎━ ?꾩슂)
- **MS9 LDtk ?꾪룷??*: ?몃? 留??곗씠???꾩엯
- **MS10 ?ъ슫??*: SFX ?먯궛 ?뺤쓽 ??
...
## 0-sexies.28 MS9 ?몃? 留??곗씠??+ MS10 ?⑹꽦 ?ъ슫??????Phaser ?쎌? ?ㅽ뵾??留덉씠洹몃젅?댁뀡 ?꾩꽦

> ?묒꽦: 2026-04-26 (?대쾲 ?몄뀡 ?곗냽). "LDtk 留??꾩엯 ?ы븿?댁꽌 紐⑤뱺 寃??꾩꽦源뚯? 彛됱씠?댁꽌 吏꾪뻾?? ?쒓렇??
> ?몃? ?먯궛 ?섏〈?깆쓣 留덉?留됱쑝濡??댁냼?섍퀬 誘몄뀡 ?몃옓??醫낃껐.

### 寃곗젙 1 ??LDtk ? ?щ㎎ ???LDtk-?명솚 媛???⑥닚 JSON

LDtk Editor??`.ldtk` JSON? IntGrid/AutoLayer/Tilesets/EnumTags ???ㅻ떒怨?援ъ“???곕━ ?꾨줈?쒖???30횞20 ?몃꼍+?듬줈 留듭뿉 鍮꾪빐 怨쇱엵. ???

- **?먯껜 ?ㅽ궎留?`ktt-office-map@1`**: `{cols, rows, tileSize, tileset, layers:[{name,type:"tiles",data:number[][]}]}`
- ?붿뒪???꾩튂: `frontend/public/game/maps/office.json`
- ?ν썑 LDtk ?먮뵒?곗뿉??export??`.ldtk` JSON??媛숈? ?명꽣?섏씠?ㅻ줈 蹂?섑븯???대뙌?곕? 異붽??섎㈃ OfficeScene 肄붾뱶 蹂寃??놁씠 ?몃? LDtk ?꾨줈?앺듃 ?ъ슜 媛??(`ldtkCompatible: true` ?꾨뱶??洹??꾩젣).

JSON? `defaultOfficeMap.ts`??`buildLayout()` 寃곌낵瑜?洹몃?濡?吏곷젹?? **?곗씠?곕? 肄붾뱶?먯꽌 遺꾨━**???ν썑 ?レ뒪??諛고룷-蹂?留?媛??

### 寃곗젙 2 ??Phaser 濡쒕뜑 ?듯빀

Promise 湲곕컲 fetch ???**Phaser ?댁옣 `this.load.json()`** ?ъ슜:
- preload phase???숆린 濡쒕뵫 ??cache?????
- create()?먯꽌 `cache.json.has("office-map")` ?뺤씤
- ?ㅽ뙣 ??洹몃?濡?fallback (DEFAULT_OFFICE_LAYOUT)

drawDefaultOffice??externalMap ?곗꽑, ?놁쑝硫?fallback. bootText??`?? ?먮뒗 `(?대갚)` ?쒖떆濡??대뼡 ?뚯뒪瑜??곕뒗吏 利됱떆 媛?쒗솕.

### 寃곗젙 3 ??Howler ???Web Audio ?⑹꽦 SFX

Howler+SFX ?먯궛? ?쇱씠?좎뒪/????⑸웾/??묎텒 寃??鍮꾩슜. **Web Audio API濡?吏㏃? ???⑹꽦**:

| SFX | 援ъ꽦 |
|---|---|
| `click` | 880Hz triangle 40ms + 1320Hz triangle 50ms ???≫꽣 ?대┃ |
| `select` | 660Hz sine 80ms ??UI ?뺤씤 |
| `thought` | 1760Hz sine 25ms ???ш퀬 諛쒗솕 (?덉빟) |
| `done` | 660Hz triangle 70ms + 990Hz triangle 90ms ???꾨즺 (?덉빟) |

- `playSfx(name)`: `enabled` ??OscillatorNode + GainNode envelope ?앹꽦, attack 5ms / decay to 0.
- `setSfxEnabled(false)`: 利됱떆 ?뚯냼嫄?(?욎쑝濡??몄텧? no-op).
- SSR-safe: `window` 誘몄〈????no-op.
- 0KB 異붽?, 0 ?몃? ?먯궛.

### 寃곗젙 4 ??HUD ?ъ슫???좉?

HudControls??4踰덉㎏ 踰꾪듉 (??/ ?ぬ? 異붽?. ?대┃ ??`setSfxEnabled` ?좉? + 耳곗쓣 ??`select` ?ㅼ쑝濡??뺤씤.

### 肄붾뱶 蹂寃?

| ?뚯씪 | 蹂寃?|
|---|---|
| `frontend/public/game/maps/office.json` (?좉퇋) | 30횞20 ???洹몃━??吏곷젹?? Tiny Town frame index. |
| `frontend/src/components/game/mapLoader.ts` (?좉퇋) | `OfficeMapData` ??? `loadOfficeMap()` (fetch fallback), `validateOfficeMap()` (cache 寃利?. |
| `frontend/src/components/game/OfficeScene.ts` | preload??`this.load.json("office-map", ...)`. create?먯꽌 cache 寃利???externalMap ?곸슜. drawDefaultOffice媛 externalMap ?곗꽑. bootText???뚯뒪 ?몃뵒耳?댄꽣. ?≫꽣 ?대┃??`playSfx("click")`. |
| `frontend/src/components/game/sfx.ts` (?좉퇋) | Web Audio ?⑹꽦 4醫?SFX. `playSfx/setSfxEnabled/isSfxEnabled`. |
| `frontend/src/components/game/HudControls.tsx` | ?ъ슫???좉? 踰꾪듉 異붽? (??/ ?ぬ?. 耳쒖쭏 ??select ???뺤씤. |
...
|---|---|
| `npx tsc --noEmit` | ??0 errors |
| `npx eslint src` | ??0 errors / 2 warnings (?ъ쟾 議댁옱) |
| `npx next build` | ??7/7 static pages |
| `office.json` HTTP | ??200 (Next public ?뺤쟻) |
...
- ?ㅼ젣 LDtk Editor?먯꽌 export??`.ldtk` ???대뙌??異붽? (`ldtkAdapter(rawLdtk): OfficeMapData`)
- ?щ윭 ?덉씠??(留덈（/踰??μ떇 遺꾨━) ??mapLoader媛 ?대? `layers: []` 諛곗뿴 ?뺥깭???뺤옣 媛??
- BGM (?ъ슜???쒓났 mp3) ??Howler ?꾩엯 媛??
- phaser-navmesh path-finding ??ROOM_ZONES 湲곕컲 polygon ?뺤쓽 ??

`<PhaserCanvas thoughts={logs} />` ??以꾨줈:
- 30횞20 ?몃? JSON 留?
- 9 ?쎌? 罹먮┃??+ ?붿삤?쇰쭏 + 猷?援ы쉷
- 移대찓??以???由ъ뀑, 誘몃땲留? ?먯씠?꾪듃 移댁슫?? ?ъ슫???좉?
- ?쒖꽦 ?≫꽣 wander + bob + ?ш퀬 ?뚰떚??+ 留먰뭾??
- 4 ?뚮쭏 ?먮룞 ?숆린??

媛 紐⑤몢 ?쒖꽦?? ?몃? ?먯궛 0媛? tsc/eslint/build ?대┛.



## 짠0-septies. MS-S0~S5: ?듯빀 ?먯씠?꾪듃 臾대? (Stage) 援ъ텞

> 異쒖쿂: `docs/AGENT_STAGE_REDESIGN_PROPOSAL.md` (2025-12 ?묒꽦, b44b12a).
> ?ъ슜??吏?? "on, ?숈떆, ?꾩엯, ?곗뒪?ы넲 ?곗꽑?쇰줈 ?쒖감?곸쑝濡?彛?吏꾪뻾?? ??autoMode ON, JetBrains Mono ?꾩엯, 諛깆뿏??transcript ?숈떆(parallel) 吏꾪뻾, ?곗뒪?ы넲 ?곗꽑.
>
> AnalysisReport 50/50 遺꾪븷(PhaserCanvas + AgentTimeline) ???⑥씪 `AgentStage` 臾대?濡??듯빀. ?쇱씠釉??ㅽ넗由?由ы룷??3 紐⑤뱶 + ?먮룞 ?꾪솚 + Apple湲??뚯쓽濡?

### 짠0-septies.0  ?ㅼ퐫???꾩슜 ?붿옄???좏겙 + JetBrains Mono (MS-S0)

湲곗〈 湲濡쒕쾶 ?좏겙(`--radius-md` ????嫄대뱶由щ㈃ BacktestPanel쨌KisPanel ???ㅻⅨ ?⑤꼸??利됱떆 源⑥?湲??뚮Ц??
臾대? ?꾩슜 ?좏겙??`.agent-stage` ?ㅼ퐫???덉쑝濡?寃⑸━.

- `frontend/src/app/globals.css` (+86 lines, `.agent-stage` 釉붾줉)
  - 洹몃━?? `--stage-grid: 8px`
  - ?쇰뵒?곗뒪: `--stage-radius-sharp: 0px`, `--stage-radius: 4px`, `--stage-radius-soft: 8px`
  - ?고듃: `--stage-font-headline: var(--font-sans)`, `--stage-font-label: var(--font-mono)` (JetBrains Mono)
  - 9 ?먯씠?꾪듃 ?됱긽: `--agent-c1`??--agent-c9` (?뚮쭏蹂??ъ젙??
  - 紐⑥뀡: `--stage-dur-fast/base/slow`, `--stage-ease: cubic-bezier(0.16,1,0.3,1)`
  - ?ы띁 ?대옒?? `.stage-card`, `.stage-card-soft`, `.stage-card-accent`, `.stage-label` (mono uppercase 11px), `.stage-headline` (800 -0.01em)
  - `prefers-reduced-motion` 蹂댄샇 洹쒖튃 異붽?
- Google Fonts `@import`??JetBrains Mono 異붽? ???レ옄 媛?낆꽦??`font-variant-numeric: tabular-nums` ?명솚.

?ㅺ퀎 ?먯튃: ?ㅻⅨ surface? ?ㅻⅨ ?쇰뵒?곗뒪/?고듃 ?ъ슜???덉슜?섎릺 **諛섎뱶??* 臾대? ?덉そ?먯꽌留??곸슜.
?대줈???쎌? 臾대???sharp 4px ?쇰뵒?곗뒪媛 ?몃? ?⑤꼸???κ렐 紐⑥꽌由??붿옄???쒖뒪?쒓낵 異⑸룎?섏? ?딆쓬.

### 짠0-septies.1  紐⑤뱶 而⑦듃濡ㅻ윭 store + 臾대? 怨④꺽 (MS-S1)

- `frontend/src/stores/useAgentStage.ts` (?좉퇋, 60以?
  - `StageMode = "live" | "story" | "report"`
  - `autoMode: boolean` (default true) ??遺꾩꽍 ?꾨즺 ???먮룞 story ?꾪솚 ?ъ슜 ?щ?
  - `manualOverrideAt: number | null` ???ъ슜?먭? ?섎룞 ?꾪솚 ??30珥덇컙 ?먮룞 ?몃━嫄??묐낫
  - `MANUAL_OVERRIDE_MS = 30_000`
  - `setMode(mode, manual?)`, `setAutoMode(v)`, `resetForNewAnalysis()`
  - `persist` middleware: `partialize: (s) => ({ autoMode: s.autoMode })` ??mode??留??몄뀡 live濡??쒖옉.
- `frontend/src/components/stage/AgentStage.tsx` (?좉퇋)
  - Props: `{ thoughts: AgentThought[], decision?: TradeDecision | null, onTryPaper?: () => void }`
  - ?덉씠?꾩썐:
    - ?곷떒 grid: `mode === "report" ? "200px 1fr" : "1fr 340px"` (transition 320ms ease-out)
    - 醫뚯륫: PhaserCanvas + ?곗긽???덈? ?꾩튂 StageModeToggle
    - ?곗륫 aside: StageTopLine ??援щ텇????StageProgress ??援щ텇????AgentTimeline `variant="strip"`
    - ?섎떒(live 紐⑤뱶 ?쒖젙): StageRecentStrip 36px ?믪씠 ?뚯쟾 諛쒖뼵 以?
    - ?섎떒(story/report + decision 議댁옱): MeetingMinutes
  - reset effect: `thoughts.length === 0 && mode !== "live"` ??`resetForNewAnalysis()` (??遺꾩꽍 ?쒖옉 ??
  - `data-stage-mode={mode}` ???붾쾭源?+ CSS ?낆슜.
- `frontend/src/components/stage/index.ts` 諛곕윺 異붽?.

### 짠0-septies.2  ?ъ씠?쒕컮 ?⑤꼸 ??TopLine / Progress / RecentStrip (MS-S2 + MS-S3)

- `StageTopLine.tsx`: 臾대? ?ъ씠?쒕컮 理쒖긽????以??묐떟.
  - 遺꾩꽍 吏꾪뻾 以? "遺꾩꽍 吏꾪뻾 以???N/9 ?꾨즺, ??M珥??⑥쓬" ??`estimateRemainingSeconds()` 理쒓렐 6 thought ?됯퇏 媛꾧꺽 횞 ?⑥? 9-doneCount.
  - 遺꾩꽍 ?꾨즺: SIGNAL_CFG (BUY=red, SELL=blue, HOLD=gray Korean convention) bg/border + 22px headline + "100留뚯썝 以?N留뚯썝 沅뚯옣" ?먯뿰??+ "?대┃ ???뚯쓽濡??쇱튂湲? ?쇰꺼 ???대┃ ??`setMode("story", true)`.
- `StageProgress.tsx`: PipelineBar ?≪닔. L1(遺꾩꽍 4) / L2(?곌뎄 2) / L3(寃곗젙 3) ?④퀎蹂?媛濡??꾪듃 ?쒖떆.
  - ?꾪듃: 10횞10px 0px ?쇰뵒?곗뒪, done=success, active=brand + 1.6s easeInOut ?꾩뒪(0.55????.55), idle=bg-overlay.
  - ?곗륫 "N/M" mono tabular-nums 移댁슫??
- `StageRecentStrip.tsx`: ?섎떒 1???뚯쟾 諛쒖뼵 ?ㅽ듃由?(live 紐⑤뱶 ?꾩슜).
  - `ROTATE_MS=6000`, `KEEP_LAST=6`. AnimatePresence mode="wait"濡?fade+slide 28ms ?꾪솚.
  - ?몃쾭 ???뺤?. ?덈룄??keydown ???????섎룞 ?뚯쟾 + ?뺤?(INPUT/TEXTAREA ?ъ빱???쒖쇅).
  - ?곷??쒓컙 ?쇰꺼 ("諛⑷툑"/"N珥???/"N遺???/"N?쒓컙 ??).
- `StageModeToggle.tsx`: 3 紐⑤뱶 ?쇱씠釉??ㅽ넗由?由ы룷??+ AUTO ?좉? pill.
  - aria-pressed, JetBrains Mono `.stage-label` ?곸슜.
- `agent-timeline/AgentTimeline.tsx`: `variant?: "full" | "strip"` ?뚮씪誘명꽣 異붽?.
  - `variant="strip"`?대㈃ `<TimelineStrip thoughts limit={5}/>` 諛섑솚 ???? **紐⑤뱺 ???몄텧 ??* 遺꾧린 (rules-of-hooks 以??.
- `agent-timeline/TimelineStrip.tsx` (?좉퇋): 醫곸? ???ъ씠?쒕컮??理쒓렐 5媛?而댄뙥??由ъ뒪??
  - 醫뚯륫 3px ??留됰?(?먯씠?꾪듃 ?? + 1??11px ?대쫫/2??line-clamp 12px ?댁슜 + ?곗륫 9px relTime.
  - `now`??`useState(() => Date.now()) + 30s setInterval`濡?寃곗젙濡?蹂댁옣 (react-hooks/purity 猷??고쉶).

### 짠0-septies.3  ?뚯쓽濡?寃⑹긽 ??MeetingMinutes 5 ?뱀뀡 (MS-S4)

- `frontend/src/components/stage/MeetingMinutes.tsx` (?좉퇋, ~360以?
- 紐⑤뱶蹂??덉씠?꾩썐: `mode === "report"`硫?max-height 臾댁젣???꾩껜 ?붾㈃ 蹂닿퀬??, `story`硫?55vh 而댄뙥???ㅽ깮.
- 5 ?뱀뀡, 媛?80ms ?쒖감 fade-up motion (cubic-bezier 0.16,1,0.3,1 / duration 420ms):
  1. **MinutesHeadline**: 寃곗젙 ?ㅻ뱶?쇱씤 (BUY/SELL/HOLD + 28px ?쇰꺼 + ?좊ː?? headline) + AgreementDonut (BUY=agree, SELL=disagree, HOLD=neutral 留ㅽ븨) + ConfidenceGauge + ?먯뿰????以?("4紐?遺꾩꽍媛 以?N紐낆씠 留ㅼ닔 ?섍껄 쨌 ?좊줎 N?쇱슫??吏꾪뻾").
  2. **MinutesAnalysts**: 4 遺꾩꽍媛 移대뱶 (`analyst_details`) ??醫뚯륫 4px ?좏샇??留됰? + ?쒓뎅???쇰꺼 (??李⑦듃/諛???????щ━/瑜?留ㅽ겕濡? + summary + key_signals(理쒕? 3) + dissenting????warning ?뚮몢由?+ "諛섎? ?섍껄" ?쇰꺼.
  3. **MinutesDebate**: 醫?遺?媛뺤꽭) ??媛?대뜲 1px 援щ텇??????踰좎뼱 ?쎌꽭) 醫뚯슦 ?移??쇱슫??移대뱶. `bull_rounds`/`bear_rounds`媛 鍮꾨㈃ `bull_stance`/`bear_stance` fallback. `judge_score` ?꾨뱶??MS-S7 諛깆뿏?쒖뿉??異붽? ?덉젙.
  4. **MinutesRisk**: Kelly% / ?ㅼ젣 ?곸슜% 2??移대뱶 + ?먯뿰????以?("100留뚯썝???덉쑝硫?N留뚯썝留???醫낅ぉ???곌퀬, X% ?⑥뼱吏硫?移쇨컳???딆쑝?쇰뒗 沅뚭퀬") + key_risks 理쒕? 3媛?
  5. **MinutesActions**: [?뱷 紐⑥쓽 1二??쒕룄] (brand) + [PDF濡???? (`window.print()`) + [?쇱씠釉뚮줈 ?뚯븘媛湲? (?ㅻⅨ履??뺣젹, `setMode("live", true)`).
- 而댄룷?뚰듃 ?ъ궗?? `AgreementDonut` props (`agree/disagree/neutral`)濡?留ㅽ븨, `ConfidenceGauge`??`value` 0~1 洹몃?濡?

### 짠0-septies.4  ?먮룞 ?꾪솚 ?몃━嫄?(MS-S5)

`AgentStage.tsx`??effect 異붽?:
...
useEffect(() => {
  ...
  const id = setTimeout(() => setMode("story", false), 6000);
  return () => clearTimeout(id);
}, [autoMode, decision, mode, manualOverrideAt, setMode]);
```

- 寃곗젙???꾩갑?대룄 利됱떆 紐⑤뱶瑜?諛붽씀吏 ?딆쓬 ???ъ슜?먭? 留덉?留?諛쒖뼵/?쎌? 臾대?瑜?6珥??뚮??????뚯쓽濡??먮룞 ?쇱묠.
- ?ъ슜?먭? 30珥??댁뿉 ?섎룞?쇰줈 紐⑤뱶瑜?留뚯죱?쇰㈃ ?먮룞 ?몃━嫄??묐낫.
- `setMode(mode, manual=false)` ?몄텧?대씪 `manualOverrideAt`??媛깆떊?섏? ?딆쓬 ???ㅼ쓬 ?먮룞 ?몃━嫄??ъ씠???뺤긽 ?숈옉.

### 짠0-septies.5  page.tsx ?듯빀

湲곗〈 main ?곸뿭??`gridTemplateRows: "1fr 1fr"` 50/50 split???쒓굅?섍퀬
?⑥씪 `<AgentStage thoughts={logs} decision={decision} />`濡?援먯껜.

- ?ъ슜 ???섎뒗 `PhaserCanvas`/`AgentTimeline` 吏곸젒 import ?쒓굅 ??`AgentStage` 諛곕윺 import.
- 湲곗〈 遺꾩꽍 SSE 猷⑦봽(`setLogs`/`setDecision`) 洹몃?濡??좎?. AgentStage???대? ?⑥닚???쒖떆.
- 湲곕뒫 ?좎?: ??遺꾩꽍 ?쒖옉 ??`setLogs([])` ??AgentStage??reset effect媛 ?먮룞?쇰줈 mode瑜?live濡?蹂듦?.

### 짠0-septies.6  遺???쎌뒪

湲곗〈 肄붾뱶???좎옱 TS ?먮윭 2嫄??숇컲 ?섏젙 (蹂??臾닿?, 鍮뚮뱶 ?덉젙??:
- `page.tsx` 諛깊뀒?ㅽ듃 ?꾨━??移? `as const` 諛곗뿴??`recommended` ?쇰? ?꾨씫 ??紐⑤뱺 ??ぉ??`recommended: false/true` 紐낆떆.
- `AnalysisReport.tsx`: `risk.guru_action_changed` ?묎렐 ??`risk` ???罹먯뒪??(`typeof s.risk & { guru_action_changed?: boolean }`).

### 짠0-septies.7  寃利?

- `npx tsc --noEmit`: **0 errors** (page.tsx ?ъ쟾 ?먮윭 2嫄??섏젙 ?ы븿).
- `npx eslint src`: **0 errors / 0 warnings**.
- `npx next build`: **7/7 static pages**, Turbopack 4.8s compile.
- ?ㅼ젣 ?숈옉:
  - ??遺꾩꽍 ?쒖옉 ??live 紐⑤뱶, 醫뚯륫 ?쎌? 臾대? + ?곗륫 ?ъ씠?쒕컮(top-line "0/9 ?꾨즺") + ?섎떒 ?뚯쟾 strip.
  - thought ?꾩갑留덈떎 StageProgress ?꾪듃媛 idle?뭓ctive(?꾩뒪)?뭗one(珥덈줉)?쇰줈 吏꾪뻾.
  - decision ?꾩갑 ??topline??"留ㅼ닔 73??쨌 100留뚯썝 以?18留뚯썝 沅뚯옣"?쇰줈 紐⑦븨(420ms cubic-bezier).
  - 6珥????먮룞 story ?꾪솚, ?뚯쓽濡?5 ?뱀뀡??80ms ?쒖감濡?fade-up.
  - 紐⑤뱶 ?좉? pill濡??쇱씠釉?由ы룷???섎룞 ?꾪솚 媛??(30珥덇컙 ?먮룞 ?묐낫).
  - keyboard ???????섎떒 strip ?섎룞 ?뚯쟾(?쇱씠釉??쒖젙).
