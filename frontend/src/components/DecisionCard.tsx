      ...
      >
        ...
        <div style={{ padding: 22 }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 18 }}>
            ...
            <div style={{ position: "relative", width: 76, height: 76 }}>
              <svg width="76" height="76" style={{ transform: "rotate(-90deg)" }}>
                <circle cx="38" cy="38" r="30" fill="none" stroke="var(--bg-elevated)" strokeWidth="6" />
                <motion.circle
              ...
              <div style={{
                ...
                <span style={{ fontSize: 17, fontWeight: 800, color: cfg.color, lineHeight: 1 }}>{confidencePct}%</span>
                <Tooltip
                  content="9개 AI 에이전트의 의견 일치도와 데이터의 명확함을 종합해 0–100%로 환산한 값. 70% 이상이면 의견이 비교적 일치함, 50% 미만이면 크게 갈렸다는 뜻. AI 판단 자체가 맞는다는 보장은 아닙니다."
                  maxWidth={300}
                >
                  <span style={{ fontSize: 11, color: "var(--text-tertiary)", marginTop: 2, borderBottom: "1px dotted var(--text-tertiary)", cursor: "help", display: "inline-flex", alignItems: "center", gap: 3 }}><Icon name="info" size={11} decorative /> 신뢰도</span>
                </Tooltip>
              </div>
            </div>
          </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                {onOpenSettings && (
                  <button
                    ...
                    style={{
                      background: "var(--bg-surface)", color: "var(--text-secondary)", fontSize: 12, fontWeight: 700,
                      padding: "7px 12px", borderRadius: "var(--radius-md)", border: "1px solid var(--border-default)",
                      cursor: "pointer",
                    }}
                  >
                    승인 정책
                ...
                {onHumanApproval && (
                  <button
                    ...
                    style={{
                      background: "var(--warning)", color: "var(--text-inverse)", fontSize: 13, fontWeight: 700,
          ...
          {guruEnabled && (
            ...
            >
              <p style={{ fontSize: 13, fontWeight: 700, color: "var(--brand-active)", marginBottom: 5 }}>
                GURU 정책이 함께 검토했어요
              </p>
              ...
              {guru?.action_changed && (
                <p style={{ fontSize: 12, color: "var(--warning)", marginTop: 6, display: "inline-flex", alignItems: "center", gap: 4 }}>
                  GURU가 액션을 조정했습니다: {guru?.llm_action} <Icon name="arrow-right" size={11} decorative /> {guru?.final_action}
                </p>
              )}
              {guruRules.length > 0 && (
            ...
            return (
              <div style={{ marginBottom: 16 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  <span style={{ fontSize: 12, color: "var(--text-tertiary)", fontWeight: 700 }}>에이전트 투표</span>
                  <span style={{ fontSize: 11, color: "var(--text-tertiary)" }}>{total}개 기준</span>
                </div>
                {/* Segmented bar */}
                  ...
                  {Object.entries(signals).map(([action, count]) => {
                    ...
                    return (
                      <div key={action} style={{
                        ...
                        <p style={{ fontSize: 11, color: "var(--text-tertiary)", marginBottom: 3, fontWeight: 600 }}>{c.label}</p>
                        <p style={{ fontSize: 20, fontWeight: 800, color: c.color, lineHeight: 1 }}>{String(count)}</p>
                      </div>
                    );
                  })}
                </div>
          ...
          <div style={{
            ...
            <p style={{ fontSize: 12, color: "var(--text-tertiary)", fontWeight: 700, marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.04em" }}>결정 근거</p>
          ...
          {(() => {
            ...
            return (
              <details style={{
                ...
                <summary style={{ fontSize: 12, color: "var(--text-secondary)", fontWeight: 700, cursor: "pointer", listStyle: "none", display: "inline-flex", alignItems: "center", gap: 5 }}>
                  <Icon name="microscope" size={13} decorative /> 결정 추적 — 어떤 에이전트가 어떻게 표를 던졌나
                </summary>
                <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
                  {Object.entries(details).map(([agent, d]) => {
                    ...
                    return (
                      <div key={agent} style={{
                        display: "grid", gridTemplateColumns: "110px 60px 1fr",
                        gap: 10, alignItems: "center",
                        padding: "7px 10px", background: "var(--bg-surface)",
                        borderRadius: "var(--radius-sm)",
                        borderLeft: `3px solid ${c.hex}`,
                      }}>
                        <span style={{ fontSize: 12, color: "var(--text-primary)", fontWeight: 600 }}>{agent}</span>
                        <span style={{ fontSize: 12, color: c.color, fontWeight: 700 }}>
                          {c.label} {Math.round(d.confidence * 100)}%
                        </span>
                        <span style={{ fontSize: 11, color: "var(--text-tertiary)", lineHeight: 1.45, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={d.summary}>
                          {d.summary}
                        </span>
                      </div>
                    );
                  ...
                  {guru?.enabled && (
                    <div style={{
                      ...
                      <p style={{ fontSize: 11, color: "var(--text-tertiary)", fontWeight: 700, marginBottom: 3, display: "inline-flex", alignItems: "center", gap: 5 }}>
                        <Icon name="graduation" size={12} decorative /> GURU 정책 ({guru.risk_profile})
                      </p>
                      <p style={{ fontSize: 12, color: "var(--text-secondary)", lineHeight: 1.6, display: "inline-flex", alignItems: "center", gap: 4, flexWrap: "wrap" }}>
                      ...
                      {/* 30일 누적 통계 (P4.23) */}
                      {guruStats.total > 0 && (
                        <p style={{ fontSize: 11, color: "var(--text-tertiary)", marginTop: 6, lineHeight: 1.55, display: "inline-flex", alignItems: "center", gap: 5 }}>
              ...
              <div style={{ background: "var(--bg-elevated)", borderRadius: "var(--radius-lg)", padding: "12px 14px" }}>
                ...
                <details style={{ marginTop: 6 }}>
                  <summary style={{ fontSize: 11, color: "var(--text-tertiary)", cursor: "pointer", listStyle: "none", display: "inline-flex", alignItems: "center", gap: 4 }}>
                    <Icon name="calculator" size={12} decorative /> 산식
                  </summary>
                  <p style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 5, lineHeight: 1.6 }}>
            ...
            {decision.agents_summary?.risk_level && (
              <div style={{ background: "var(--bg-elevated)", borderRadius: "var(--radius-lg)", padding: "12px 14px" }}>
                <Tooltip
                  ...
                  maxWidth={300}
                >
                  <p style={{ fontSize: 12, color: "var(--text-tertiary)", borderBottom: "1px dotted var(--text-tertiary)", display: "inline-flex", alignItems: "center", gap: 3, cursor: "help" }}><Icon name="info" size={11} decorative /> 리스크 등급</p>
                </Tooltip>
                <p style={{ fontSize: 18, fontWeight: 700, color: "var(--text-primary)", marginTop: 4 }}>{decision.agents_summary.risk_level}</p>
              </div>
            )}
            {typeof stopLossPct === "number" && (
              <div style={{ background: "var(--bg-elevated)", borderRadius: "var(--radius-lg)", padding: "12px 14px" }}>
                <Tooltip
                  ...
                  maxWidth={300}
                >
                  <p style={{ fontSize: 12, color: "var(--text-tertiary)", borderBottom: "1px dotted var(--text-tertiary)", display: "inline-flex", alignItems: "center", gap: 3, cursor: "help" }}><Icon name="info" size={11} decorative /> 손절 라인</p>
                </Tooltip>
                <p style={{ fontSize: 18, fontWeight: 700, color: "var(--text-primary)", marginTop: 4 }}>{stopLossPct}%</p>
              </div>
            )}
            {decision.agents_summary?.entry_strategy && (
              <div style={{ gridColumn: "1 / -1", background: "var(--bg-elevated)", borderRadius: "var(--radius-lg)", padding: "12px 14px" }}>
                <p style={{ fontSize: 12, color: "var(--text-tertiary)", marginBottom: 4, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.04em" }}>진입 전략</p>
                <p style={{ fontSize: 13, color: "var(--text-primary)", lineHeight: 1.65 }}>{decision.agents_summary.entry_strategy}</p>
              </div>
            )}
            {decision.agents_summary?.exit_strategy && (
              <div style={{ gridColumn: "1 / -1", background: "var(--bg-elevated)", borderRadius: "var(--radius-lg)", padding: "12px 14px" }}>
                <p style={{ fontSize: 12, color: "var(--text-tertiary)", marginBottom: 4, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.04em" }}>청산 전략</p>
                <p style={{ fontSize: 13, color: "var(--text-primary)", lineHeight: 1.65 }}>{decision.agents_summary.exit_strategy}</p>
              </div>
            )}
          </div>
          ...
          {(onGoTrading || onGoBacktest || onGoAutoLoop) && (
            <div style={{ marginTop: 14, paddingTop: 14, borderTop: "1px dashed var(--border-subtle)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: 10 }}>
                {onGoTrading && (
                  <button
                    onClick={onGoTrading}
                    style={{
                      padding: "12px 10px", borderRadius: "var(--radius-lg)", border: "1px solid var(--border-default)",
                      background: "var(--bg-elevated)", color: "var(--text-primary)", fontSize: 13, fontWeight: 700, cursor: "pointer",
                      display: "flex", flexDirection: "column", alignItems: "center", gap: 6, lineHeight: 1.3,
                    }}
                  >
                    <Icon name="edit" size={18} decorative />
                    <span>모의로 1주 시도</span>
                    <span style={{ fontSize: 11, fontWeight: 500, color: "var(--text-tertiary)" }}>실제 돈 ×</span>
                  </button>
                )}
                {onGoBacktest && (
                  <button
                  ...
                  >
                    ...
                    <span>백테스트로 검증</span>
                    <span style={{ fontSize: 11, fontWeight: 500, color: "var(--text-tertiary)" }}>과거 성과 확인</span>
                  </button>
                )}
                {onGoAutoLoop && (
                  <button
                    ...
                    style={{
                      padding: "12px 10px", borderRadius: "var(--radius-lg)", border: `1px solid ${cfg.color}`,
                      background: cfg.bg, color: cfg.color, fontSize: 13, fontWeight: 700, cursor: "pointer",
                      display: "flex", flexDirection: "column", alignItems: "center", gap: 6, lineHeight: 1.3,
                    }}
                  >
                    <Icon name="robot" size={18} decorative />
                    <span>자동매매 설정</span>
                    <span style={{ fontSize: 11, fontWeight: 500, opacity: 0.85 }}>루프에 접속</span>
                  </button>
                )}
              </div>
            </div>
