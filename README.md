...
- **JSON 형식 강제 + 안전 파서**. 코드 블록 제거, 첫 `{`부터 마지막 `}`까지 추출, 실패 시 안전한 fallback으로 복구합니다.
...
- **다크/라이트/시스템 테마**. FOUC를 막는 inline 스크립트와 Linear/Vercel/Toss 계열 팔레트를 함께 적용했습니다.
...
- **live**. `data.kis.trading.place_order()`를 호출합니다. 잔고 조회로 실제 `cash/shares`를 확인하고, 보수적인 fallback도 함께 처리합니다.
...
AutoLoop은 위 4단계를 거치지 않습니다. 서버가 자동 실행되기 때문입니다. 대신 **GURU/리스크 매니저의 `requires_human_approval=true`가 나오면 주문 자체를 보류**하고, UI 알림으로 표시합니다. `guru_require_user_confirmation=true`로 설정하면 모든 BUY/SELL이 자동으로 보류됩니다.
