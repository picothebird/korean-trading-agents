...
- **무엇을 하는가** — 종목 하나를 입력하면
  ① 4인 분석가 패널(기술 · 재무 · 감성 · 거시)이 동시에 분석 →
...
| OpenDART API Key | 무료 발급 — 공시·재무 데이터 + 직전 6개 보고서 시계열 |
| BOK ECOS API Key | 무료 발급 — 한국은행 공식 금리·외국인 수급·거시 100대 지표 |
...
BOK_API_KEY=...                  # https://ecos.bok.or.kr/api/ 무료 발급 (한국은행 거시·수급)
...
> **데이터 윈도우 & 시점 라벨링** — 모든 분석가 프롬프트는 `[분석 기준 시각]` 헤더로 데이터 시점을 명시합니다.
> 윈도우는 학술 표준(Jegadeesh-Titman 1993 / Piotroski 2000 / Tetlock 2007 / Choe-Kho-Stulz 1999 / Estrella-Mishkin 1996)을 따릅니다.
> 자세한 매핑은 [docs/DEVELOPER_GUIDE.md §9.2](docs/DEVELOPER_GUIDE.md) 참조.
