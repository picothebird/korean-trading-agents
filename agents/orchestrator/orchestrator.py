...
async def risk_manager(
    ...
    await emit_thought(session_id, AgentThought(
        ...
        content="포트폴리오 리스크 & Half-Kelly 포지션 분석 중...",
    ))
...
requires_human_approval=true 조건: 신뢰도 80% 이상이고 포지션 20% 초과, 또는 위험도 CRITICAL"""
    ...
    await emit_thought(session_id, AgentThought(
        ...
        role=AgentRole.RISK_MANAGER,
        status=AgentStatus.DONE,
        content=f"리스크: {result.get('risk_level')} | Half-Kelly: {kelly_pct}% | 승인: {result.get('approval')}",
        metadata=result,
    ))
    return result
...
Half-Kelly 권장 포지션: {kelly_pct}% | 최대 허용: {risk_result.get('max_position_pct')}%
...
Half-Kelly · 리스크 한도 적용 결과(권장 비중): {position_pct:.1f}%
...
JSON: {{"action": "BUY|SELL|HOLD", "confidence": 0.0~1.0, "reasoning": "결정 근거 — 충분한 말량 (제한 없음, 800자 권장)", "position_size_pct": {position_pct:.1f}, "entry_strategy": "진입 전략 (위 규칙 준수, 분할 합계 = position_size_pct)", "exit_strategy": "청산 전략 (목표가, 손절가)"}}"""
    ...
    # ── 진입 비중 일관성 강제 ────────────────────────────────────
    # LLM 이 본문에 다른 비중(예: "16% 한도")을 적는 케이스가 있어,
    # 결정값(position_size_pct)·entry_strategy 모두 Kelly·max 제약(=position_pct)과
    # 일치하도록 시스템에서 정정한다. 차이가 0.5%p 미만이면 그대로 둔다.
    canonical_position_pct = round(float(position_pct), 1)
    try:
        llm_position_pct = round(float(pm_result.get("position_size_pct", 0) or 0), 1)
    except (TypeError, ValueError):
        llm_position_pct = 0.0
    if abs(llm_position_pct - canonical_position_pct) >= 0.5:
        pm_result["position_size_pct"] = canonical_position_pct
        original_entry = str(pm_result.get("entry_strategy") or "").strip()
        prefix = f"[총 {canonical_position_pct:.1f}% 한도 — 시스템 정정]"
        if original_entry:
            pm_result["entry_strategy"] = f"{prefix}\n{original_entry}"
        else:
            pm_result["entry_strategy"] = prefix
    else:
        pm_result["position_size_pct"] = canonical_position_pct
    ...
    status_msg = f"🎯 최종: {decision.action} | 신뢰도: {decision.confidence*100:.0f}% | 권장 포지션: {pm_result.get('position_size_pct', 0)}% (Half-Kelly 산식 {kelly_pct}%)"
    if requires_human:
...
권장 비중: {summary.get('position_size_pct', 0)}% (Half-Kelly 산식 {risk_block.get('kelly_position_pct', 0)}% → 리스크·정책 한도 적용)
리스크: {risk_block.get('risk_level','-')} / 손절 {risk_block.get('stop_loss_pct','-')}%
진입: {summary.get('entry_strategy') or '-'}
청산: {summary.get('exit_strategy') or '-'}
...
- situation_today: 시장·기업·뉴스·거시경제 상황을 600~1000자 산문으로 풀어주세요
...
async def run_analysis(
    ...
    try:
        ...
        # 아티클 실패는 의사결정을 막지 않도록 조용히 닫는다. 원인은 로그 확인.
        print(f"[article_writer:outer] swallowed: {exc!r}", file=sys.stderr, flush=True)
